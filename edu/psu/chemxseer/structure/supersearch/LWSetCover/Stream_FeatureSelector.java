...
public class Stream_FeatureSelector {
	...
	public NoPostingFeatures<OneFeatureMultiClass> minePrefixFeatures(
		...
		while (index < K && (nextPattern = patternGenerator.nextPattern())!=null) {
			...
			timeDuration += System.currentTimeMillis() - start;
		}
		MemoryConsumptionCal.runGC();
		...
		while ((nextPattern = patternGenerator.nextPattern()) != null) {
			CoverSet_Weighted newSet = LWIndexSetConverter.
					featureToSet_Weight(patternGenerator.getFeature(nextPattern), gDB, querySize);
			start = System.currentTimeMillis();
			if (minSetID == -1) {
			...
			if (newGain1KTimes > 2 * minSetScore1KTimes) {
				// do swap: System.out.println("Swap " + newGain + " with " + minSetID + " and score " + minSetScore);
				this.removeSelectedSet(selectedSets[minSetID], status, statusList);
		...
		System.out.println("(4) Time for Pattern Selection:" + timeDuration);
		System.out.println("(5) After Pattern Selection:" + K + " out of "
				+ patternGenerator.getEnumeratedPatternNum() + " frequent patterns as selected for Indexing");
