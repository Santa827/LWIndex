...
import edu.psu.chemxseer.structure.setcover.IO.features.FeatureWrapperOnDiskImpl;
import edu.psu.chemxseer.structure.setcover.IO.features.FeatureWrapperInterface;
...
public class MaxCover_FeatureSelector {
	...
	public NoPostingFeatures<OneFeatureMultiClass> minePrefixFeatures(
		...
		NoPostingFeatures<OneFeatureMultiClass> features = frequentSubgraphs.getMultiFeatures();
		CoverSet_Weighted[] sets = new CoverSet_Weighted[features.getfeatureNum()];
		int querySize = frequentSubgraphs.getClassGraphsCount()[1];
		for (int i = 0; i < sets.length; i++) {
			features.getFeature(i).setUnselected();
			FeatureWrapperInterface oneFeature = new FeatureWrapperOnDiskImpl(frequentSubgraphs, i, -1);
			sets[i] = LWIndexSetConverter.featureToSet_Weight(oneFeature, gDB, querySize);
		}
		System.out.println("Time for Loading & Converting Features:" + (System.currentTimeMillis() - start));
		...
		System.out.println("(3) Space for Pattern Selection: " + (memoryEnd - memoryStart));
		System.out.println("(4) Time for Pattern Selection: " + (System.currentTimeMillis() - start));
		System.out.println("(5) After Pattern Selection: " + selectedFeatureCount + " out of " + sets.length
				+ " frequent patterns as selected for Indexing");
		// 4. Return selected Features
		List<OneFeatureMultiClass> selectedFeatures = features.getSelectedFeatures();
		return new NoPostingFeatures<OneFeatureMultiClass>(selectedFeatures,true);
	}
	...
	public NoPostingFeatures<OneFeatureMultiClass> minePrefixFeaturesWithInvertedIndex(
		...
		int querySize = frequentSubgraphs.getClassGraphsCount()[1];
		for (int i = 0; i < sets.length; i++) {
			FeatureWrapperInterface oneFeature = new FeatureWrapperOnDiskImpl(
