...
import edu.psu.chemxseer.structure.setcover.IO.features.FeatureWrapperSimpleImpl;
...
public class GSpanMiner_MultiClass_Iterative extends AbstractMiner {
	...
	private boolean chooseAllDistinctEdges;
	...
	/**
	 * Return the graph edge count of graph in dimension "d"
	 * For example, dimension = 0 means database graph
	 * dimension = 1 means query graph 
	 * @param dimension
	 * @return
	 */
	public int[] getGraphEdgeCount(int dimension){
		//TODO: make sure that the order of the readed graph does not change w.r.t the original input order
		int[] result = new int[classGraphsCount[dimension]];
		int gID = 0;
		for (Object temp : this.m_graphs) {
			ClassifiedGraph g = (ClassifiedGraph) temp;
			float[] freq = g.getClassFrequencies();
			if(freq[dimension]!=0)
				result[gID++] = g.getEdgeCount();
		}
		return result;
	}
	/*
	 ...
	 */
	public GSpanMiner_MultiClass_Iterative(Settings settings, boolean chooseAllDistinctEdges) {
		super(settings);
		...
		this.chooseAllDistinctEdges = chooseAllDistinctEdges;
	...
	protected void startRealMining() {
		...
		// ADDED By DAYU
		// This is added in order to return all the distinct edges as features
		if(chooseAllDistinctEdges){
			float[] dayuFrequency = new float[] { 1f, 0f };
			gs = new DataBase(m_graphs, dayuFrequency, m_frequentSubgraphs, factory);
		}
		else 
			gs = new DataBase(m_graphs, m_settings.minimumClassFrequencies, m_frequentSubgraphs, factory);
	 ...
	 * @return
	 */
	public FeatureWrapperSimpleImpl getFeature(FrequentFragment pattern) {
		// 1. First Step:
		...
		return new FeatureWrapperSimpleImpl(feature, thePostings);
	}

	/************************** BELOW IS THE STATIC FUNCTION USED TO CALL **********************************/
	/*****************************************************************************************************/
	public static GSpanMiner_MultiClass_Iterative getMiner(String[] args, boolean chooseAllDistinctEdges) {
		Settings s = parseInput(args);
		s.debug = -1;
		GSpanMiner_MultiClass_Iterative m = new GSpanMiner_MultiClass_Iterative(s, chooseAllDistinctEdges);
		return m;
	}
	...
	public static PostingFeaturesMultiClass gSpanMining(String[] args,
			String featureFileName, String[] postingFileNames, boolean chooseAllDistinctEdges) {
		// 1. Parse the input
		Settings s = parseInput(args);
		// 2. Mining
	...
	/**
	 * Only Effective when chooseAllDistinctEdges = true
	 * Otherwise, return all frequent edges
	 * @return
	 */
	...
	public static GSpanMiner_MultiClass_Iterative getMiner(String[] args,
			List<ClassifiedGraph> inputGraphs, boolean chooseAllDistinctEdges) {
		Settings s = parseInput(args);
		s.debug = -1;
		GSpanMiner_MultiClass_Iterative m = new GSpanMiner_MultiClass_Iterative(
				s, inputGraphs, chooseAllDistinctEdges);
		return m;
	}
	
	public PostingFeaturesMultiClass gSpanMining(String featureFileName, String[] postingFileNames){
		long startTime = 0;
		float[] freq = this.m_settings.minimumClassFrequencies;
		for (float oneFreq : freq) {
			System.out.print("minFrequency: " + oneFreq + ", ");
		}
		System.out.println();

		this.iterativeMining();
		System.out.println("The Total Time Complexity: "
				+ (System.currentTimeMillis() - startTime));
		System.out.println("The Total Number of Patterns Enumerated: "
				+ this.numberOfPatterns);
		System.out.println("The Total Number of Patterns Found: "
				+ this.m_frequentSubgraphs.size());
		// 3. return
		try {
			return getFeatures(this, m_settings, featureFileName, postingFileNames);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public int getFeatureSizeUpperBound() {
		return this.m_settings.maximumFragmentSize;
	}
